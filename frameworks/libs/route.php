 ...
 * url路由器
 * 
 * 针对我们的mvc结构，路由规则存在公共配置和APP配置两种：
 *  公共配置存在于mvc/route.inc.php文件中，公共配置也就是惯例配置
 *  APP配置存在于自身APP目录的route.inc.php文件中
 * 
 * 路由规则应该参考REST设计思想
 * 
 * @copyright (c) 7725, Melon
 */
class route {
    ...
    private function _setMethod($method) {
        $this->_method = 'action'.ucfirst(strtolower($method));
    }
