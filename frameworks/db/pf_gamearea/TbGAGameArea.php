...
class TbGAGameArea {
	...
	public function /*boolean*/ delete()
	{
		...
		$sql = "DELETE FROM `GA_GameArea` WHERE `ga_AreaID`='{$this->ga_AreaID}'";
	...
	private function /*string*/ getUpdateFields()
	{
		...
		if ($this->is_ga_AreaIndex_dirty)
		{			
			if (!isset($this->ga_AreaIndex))
			{
				$update .= ("`ga_AreaIndex`=null,");
			}
			else
			{
				$update .= ("`ga_AreaIndex`='{$this->ga_AreaIndex}',");
			}
		}
